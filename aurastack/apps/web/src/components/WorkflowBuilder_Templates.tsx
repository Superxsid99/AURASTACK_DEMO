...
export const useWorkflowTemplates = (
  setWorkflow: React.Dispatch<React.SetStateAction<WorkflowDefinition>>,
  ...
  const applyTemplate = (templateName: string) => {
    const templates: Record<string, WorkflowDefinition> = {
      ...
      'Policy Servicing': {
        ...
        last_updated: new Date().toISOString(),
        nodes: [
          { id: 'start', type: 'input', label: 'Service Request', x: 50, y: 150, config: {}, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'service.request', produces_event: 'service.intake_complete', writes: ['request_details'] }, next: ['validate'] },
          ...
          { id: 'update', type: 'api_call', label: 'Record Update', x: 510, y: 150, config: { endpoint: '/api/v1/update-policy' }, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'service.validated', produces_event: 'service.updated', writes: ['update_confirmation'] }, next: ['notify'] },
          { id: 'notify', type: 'api_call', label: 'Confirmation', x: 740, y: 150, config: { endpoint: '/api/v1/notify' }, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'service.updated', produces_event: 'service.notified', writes: ['notification_id'] }, next: ['end'] },
          { id: 'end', type: 'end', label: 'Service Complete', x: 970, y: 150, config: {}, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'service.notified', produces_event: 'service.finalized', writes: ['final_status'] }, next: [] }
        ]
      },
      'Policy Renewals': {
        ...
        last_updated: new Date().toISOString(),
        nodes: [
          { id: 'start', type: 'input', label: 'Renewal Trigger', x: 50, y: 150, config: {}, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'renewal.trigger', produces_event: 'renewal.intake_complete', writes: ['renewal_data'] }, next: ['eligibility'] },
          ...
          { id: 'offer', type: 'api_call', label: 'Renewal Offer', x: 970, y: 150, config: { endpoint: '/api/v1/send-offer' }, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'renewal.adjusted', produces_event: 'renewal.offered', writes: ['offer_id'] }, next: ['end'] },
          { id: 'end', type: 'end', label: 'Renewal Processed', x: 1200, y: 150, config: {}, execution: { execution_type: 'system_action', department: 'POLICY_ISSUANCE', consumes_event: 'renewal.offered', produces_event: 'renewal.finalized', writes: ['final_status'] }, next: [] }
        ]
      },
      'Compliance Audit': {
    ...
    if (templates[templateName]) {
      setWorkflow(templates[templateName]);
