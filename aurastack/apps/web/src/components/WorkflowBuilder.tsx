...
export const WorkflowBuilder: React.FC<WorkflowBuilderProps> = ({ 
  ...
  const [validationWarnings, setValidationWarnings] = useState<{ id: string; message: string }[]>([]);
  ...
  const validateWorkflow = () => {
    ...
    if (inputNodes.length === 0) {
      errors.push({ id: 'workflow', message: 'Workflow must have at least one input node.' });
    }
    if (inputNodes.length > 1) {
      warnings.push({ id: 'workflow', message: `Workflow has ${inputNodes.length} input nodes; consider a single canonical entry.` });
    }
    ...
    workflow.nodes.forEach(node => {
      node.next.forEach((targetId) => {
        if (!nodeIds.has(targetId)) {
          errors.push({ id: node.id, message: `Node "${node.label}" links to missing node "${targetId}".` });
        }
        if (targetId === node.id) {
          errors.push({ id: node.id, message: `Node "${node.label}" cannot point to itself.` });
        }
      });
      ...
      if (node.type === 'decision' && node.next.length < 2) {
        errors.push({ id: node.id, message: `Decision "${node.label}" should have at least two outgoing paths.` });
      }

      // Execution Layer Validation
      if (!node.execution.produces_event) {
        warnings.push({ id: node.id, message: `Node "${node.label}" does not produce an event.` });
      }
      if (node.execution.writes.length === 0 && node.type !== 'end') {
        warnings.push({ id: node.id, message: `Node "${node.label}" has no explicit write operations.` });
      }
    ...
    // 4. Reachability from first input node
    ...
    if (start) {
      ...
      workflow.nodes.forEach((node) => {
        if (!visited.has(node.id)) {
          warnings.push({ id: node.id, message: `Node "${node.label}" is unreachable from the main input path.` });
        }
      });
    }
      ...
      <header className="min-h-14 border-b border-outline-variant/10 flex items-stretch bg-surface-container-low z-20 min-w-0">
        ...
        <div className="flex-1 min-w-0 flex items-center py-1.5 pr-3 pl-2">
          ...
          <button 
            ...
            className="btn btn-muted shrink-0 sm:px-4"
          >
            <Shield size={14} className={`shrink-0 ${validationErrors.length > 0 ? 'text-error' : validationWarnings.length > 0 ? 'text-amber-500' : 'text-primary'}`} />
            Validate {validationErrors.length > 0 ? `(${validationErrors.length})` : validationWarnings.length > 0 ? `(${validationWarnings.length} warn)` : ''}
          </button>
          <button 
            type="button"
        ...
        <aside className="w-64 border-r border-outline-variant/10 bg-surface-container-low p-4 flex flex-col gap-6 z-10 overflow-y-auto custom-scrollbar">
          <div>
            {(validationErrors.length > 0 || validationWarnings.length > 0) && (
              <div className={`mb-4 rounded-sm border p-2 ${validationErrors.length > 0 ? 'border-error/40 bg-error/10' : 'border-amber-500/40 bg-amber-500/10'}`}>
                <div className={`text-[9px] font-bold uppercase tracking-widest ${validationErrors.length > 0 ? 'text-error' : 'text-amber-400'}`}>
                  Graph Health: {validationErrors.length > 0 ? `${validationErrors.length} error(s)` : 'No errors'}{validationWarnings.length > 0 ? `, ${validationWarnings.length} warning(s)` : ''}
                </div>
                {validationErrors.slice(0, 2).map((issue, idx) => (
                  <p key={`err-${idx}`} className="mt-1 text-[9px] text-on-surface-variant leading-relaxed">{issue.message}</p>
                ))}
                {validationWarnings.slice(0, 1).map((issue, idx) => (
                  <p key={`warn-${idx}`} className="mt-1 text-[9px] text-on-surface-variant leading-relaxed">{issue.message}</p>
                ))}
              </div>
            )}
                  ...
                  {heatmapMode && (
                    <>
                      <Rect
                        width={NODE_WIDTH}
                        height={NODE_HEIGHT}
                        fill={getNodeHeatOverlay(node)}
                        cornerRadius={4}
                      />
                      <Text
                        text={getNodeHeatLabel(node)}
                        x={NODE_WIDTH - 64}
                        y={8}
                        fill="#ffffffcc"
                        fontSize={8}
...
const getNodeHeatOverlay = (node: Node) => {
  const runtime = node.config?.runtime;
  const totalRuns = Number(runtime?.totalRuns || 0);
  const failedRuns = Number(runtime?.failedRuns || 0);
  const successRate = Number(runtime?.successRate || 0);

  if (totalRuns <= 0) return '#64748b26';
  if (failedRuns > 0 && successRate < 70) return '#ef444455';
  if (successRate < 85) return '#f59e0b44';
  return '#10b98133';
};

const getNodeHeatLabel = (node: Node) => {
  const runtime = node.config?.runtime;
  const totalRuns = Number(runtime?.totalRuns || 0);
  const successRate = Number(runtime?.successRate || 0);
