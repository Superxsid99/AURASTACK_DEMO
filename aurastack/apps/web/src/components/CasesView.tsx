...
export const CasesView: React.FC<CasesViewProps> = ({ cases, setCases, tasks, logs, workflows, agents, selectedCaseId: propSelectedCaseId, setSelectedCaseId, setActiveTab }) => {
  ...
  const filteredCases = cases.filter(c => {
    ...
    const matchesSearch = title.toLowerCase().includes(search.toLowerCase()) || id.toLowerCase().includes(search.toLowerCase());
    ...
    >
      <div className="w-24 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="text-xs font-sans font-bold text-primary">{caseData.id || 'N/A'}</div>
        </div>
      </div>

      <div className="w-32">
        <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Department</div>
        <div className="text-[10px] font-bold text-on-surface uppercase tracking-tighter">{String(caseData.department || 'CLAIMS').replace('_', ' ')}</div>
      </div>

      <div className="flex-1">
        <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">{caseData.type || 'CLAIM'}</div>
        <div className="text-sm font-bold text-on-surface">{caseData.title || 'Untitled Case'}</div>
      </div>
...
const getRequiredDocumentMatchers = (type: Case['type']): Array<{ label: string; matches: string[] }> => {
  switch (type) {
    case 'CLAIM':
      return [
        { label: 'Medical Records', matches: ['medical', 'report', 'discharge', 'lab'] },
        { label: 'Proof of Loss', matches: ['claim', 'loss', 'invoice', 'bill'] },
        { label: 'Identity Proof', matches: ['id', 'identity', 'aadhaar', 'pan', 'passport'] },
        { label: 'Policy Document', matches: ['policy'] }
      ];
    default:
      return [];
  }
...
const CaseDetailView = ({ 
  ...
  const caseTabStorageKey = `aurastack.case.tab.${caseData.id}`;
  const tabConfig = [
    { label: 'Overview', key: 'overview' },
    { label: 'Documents', key: 'documents' },
    { label: 'Analysis', key: 'analysis' },
    { label: 'Tasks', key: 'tasks' },
    { label: 'Audit & Compliance', key: 'audit' }
  ] as const;
  const tabByKey = new Map(tabConfig.map((tab) => [tab.key, tab.label]));
  const tabByLabel = new Map(tabConfig.map((tab) => [tab.label, tab.key]));
  const queryTabKey = String(searchParams.get('tab') || '').toLowerCase();
  const storedTabLabel = (() => {
    const storedKey = window.sessionStorage.getItem(caseTabStorageKey);
    return storedKey ? tabByKey.get(storedKey) : undefined;
  })();
  const initialTabFromQuery = tabByKey.get(queryTabKey) || storedTabLabel || 'Overview';
  const [activeTab, setActiveTab] = useState(initialTabFromQuery);
  const [selectedDoc, setSelectedDoc] = useState<any>(null);

  useEffect(() => {
    const tabKey = String(searchParams.get('tab') || '').toLowerCase();
    const resolvedLabel = tabByKey.get(tabKey);
    if (resolvedLabel) {
      if (resolvedLabel !== activeTab) {
        setActiveTab(resolvedLabel);
      }
      return;
    }

    const storedKey = window.sessionStorage.getItem(caseTabStorageKey);
    const storedLabel = storedKey ? tabByKey.get(storedKey) : undefined;
    if (storedLabel && storedLabel !== activeTab) {
      setActiveTab(storedLabel);
    }
  }, [searchParams, activeTab, caseTabStorageKey]);

  useEffect(() => {
    const key = tabByLabel.get(activeTab);
    if (!key) return;
    window.sessionStorage.setItem(caseTabStorageKey, key);
  ...
  }, [activeTab, caseTabStorageKey, searchParams, setSearchParams]);
  ...
  const docStats = (caseData.metadata as any)?.docStats as { total?: number; complete?: number; missing?: number } | undefined;
  const semanticMatchers = getRequiredDocumentMatchers(caseData.type);
  const missingDocs =
    semanticMatchers.length > 0
      ? semanticMatchers
          .filter((rule) => {
            const hasMatch = uploadedDocs.some((doc) => {
              const source = normalize(`${doc.name} ${doc.type}`);
              return rule.matches.some((token) => source.includes(token));
            });
            return !hasMatch;
          })
          .map((rule) => rule.label)
  ...
  const docsUploadedCount = typeof docStats?.total === 'number' ? docStats.total : uploadedDocs.length;
  const docsMissingCount = typeof docStats?.missing === 'number' ? docStats.missing : missingDocs.length;
  ...
  const workflow = workflows.find(w => w.workflow_id === caseData.workflow_id);
  const extractedFields = ((caseData.metadata as any)?.extractedFields || {}) as Record<string, any>;
  const extractionRows = (caseData.document_extraction_status && caseData.document_extraction_status.length > 0)
    ? caseData.document_extraction_status
    : [
        { id: 'docs', type: 'Documents Received', progress: docsUploadedCount > 0 ? 100 : 0, status: docsUploadedCount > 0 ? 'COMPLETED' : 'PENDING' },
        { id: 'ocr', type: 'OCR & Extraction', progress: extractedFields && Object.keys(extractedFields).length > 0 ? 100 : 0, status: extractedFields && Object.keys(extractedFields).length > 0 ? 'COMPLETED' : 'PENDING' },
        { id: 'validation', type: 'Validation', progress: caseData.compliance_score || 0, status: (caseData.compliance_score || 0) >= 75 ? 'COMPLETED' : 'IN_PROGRESS' }
      ];
  const complianceStatusLabel =
    (caseData.compliance_score || 0) >= 90
      ? 'Fully Compliant'
      : (caseData.compliance_score || 0) >= 70
        ? 'Review Recommended'
        : 'Non-Compliant';
  const clinicalInsight =
    caseData.ai_insights?.find((insight) => insight.type === 'RISK')?.description ||
    caseData.summary ||
    'AI insight will appear once OCR and downstream analysis complete.';
  const modelConfidence = Math.max(
    1,
    Math.round(
      (caseData.ai_insights && caseData.ai_insights.length > 0
        ? caseData.ai_insights.reduce((max, insight) => Math.max(max, insight.confidence || 0), 0)
        : 0.75) * 100
    )
  );
  const routingDecision = (((caseData.metadata as any)?.routingDecision && typeof (caseData.metadata as any)?.routingDecision === 'object')
    ? ((caseData.metadata as any).routingDecision as Record<string, any>)
    : null);
            ...
            >
              <div className="grid grid-cols-12 gap-6">
                      ...
                      {hasPermission('VIEW_MEDICAL_RECORDS') ? (
                        <div className="space-y-4">
                          {(caseData.risk_markers && caseData.risk_markers.length > 0
                            ? caseData.risk_markers
                            : [{ id: 'default', type: 'Risk', status: 'INFO' as const, description: 'No active risk markers reported.' }]
                          ).map((marker) => (
                            <div key={marker.id} className="flex justify-between items-start gap-4 pb-3 border-b border-outline-variant/5 last:border-0 last:pb-0">
                              <div>
                                <div className="text-xs font-bold text-on-surface uppercase tracking-tighter">{marker.type}</div>
                                <div className="text-[10px] text-on-surface-variant">{marker.description}</div>
                              </div>
                        ...
                        <div className="space-y-4">
                          {extractionRows.map((row) => (
                            <div key={row.id} className="flex items-center justify-between">
                              <span className="text-xs font-bold text-on-surface uppercase tracking-tighter">{row.type}</span>
                              <div className="w-24 h-1 bg-surface-container rounded-full overflow-hidden">
                                <div className={`h-full ${row.status === 'FAILED' ? 'bg-error' : 'bg-primary'}`} style={{ width: `${Math.max(0, Math.min(100, row.progress || 0))}%` }}></div>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                <div className="col-span-4 space-y-6">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <div className={`text-xs font-bold uppercase tracking-tighter ${
                            complianceStatusLabel === 'Fully Compliant'
                              ? 'text-emerald-500'
                              : complianceStatusLabel === 'Review Recommended'
                                ? 'text-amber-500'
                                : 'text-error'
                          }`}>
                  ...
                  {routingDecision && (
                    <div className="card-elevated p-6 border border-primary/20">
                      <h3 className="text-[10px] font-bold uppercase tracking-widest text-primary mb-4">Workflow Routing</h3>
                      <div className="space-y-3 text-xs">
                        <div>
                          <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Selected</div>
                          <div className="font-bold text-on-surface">
                            {String(routingDecision.selectedWorkflowName || caseData.workflow_id || 'Unknown')}
                          </div>
                        </div>
                        <div>
                          <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Source</div>
                          <div className="text-on-surface-variant">
                            {String(routingDecision.source || 'unknown').replace(/_/g, ' ')}
                          </div>
                        </div>
                        <div>
                          <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Confidence</div>
                          <div className="font-bold text-emerald-400">
                            {Math.round(Number(routingDecision.confidence || 0) * 100)}%
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-primary-container/20 border border-primary/20 backdrop-blur-xl p-6 rounded-lg relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-primary/10">
                      ...
                      <span className="text-sm font-black text-primary">{modelConfidence}%</span>
                    </div>
                  </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-widest opacity-60">
                    {docsUploadedCount} Uploaded • {docsMissingCount} Missing
                  </p>
                </div>
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-on-surface-variant uppercase tracking-widest opacity-80">
                        {missingDocs.length > 0
                          ...
                          : `Some required documents are pending verification (${docsMissingCount} remaining).`}
                      </div>
                    </div>
                  </div>
                  <button 
                    ...
                    onClick={() => {
                      toast.promise(
                        new Promise(resolve => setTimeout(resolve, 1500)),
                        {
                          ...
                          success: `Request for ${docsMissingCount} documents sent to patient.`,
