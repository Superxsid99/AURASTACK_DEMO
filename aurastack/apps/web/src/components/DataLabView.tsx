...
export const DataLabView: React.FC<DataLabViewProps> = ({ onImport }) => {
  ...
  const [routingInsights, setRoutingInsights] = useState<RoutingInsight[]>([]);
  const [isLoadingDataLab, setIsLoadingDataLab] = useState(false);
  ...
  useEffect(() => {
    ...
    const readJson = async (response: Response) => {
      ...
      if (!response.ok) {
        throw new Error(`Data Lab API failed (${response.status})`);
      }
      if (!contentType.includes('application/json')) {
        throw new Error('Data Lab API returned non-JSON response. Please restart web server after proxy changes.');
      }
      return response.json();
    };
    const loadDataLab = async () => {
      setIsLoadingDataLab(true);
      setDataLabError(null);
      try {
        const [docsRes, qualityRes, routingRes] = await Promise.all([
          fetch('/api/datalab/policy-docs'),
          fetch('/api/datalab/quality-summary'),
          fetch('/api/datalab/workflow-routing-insights?limit=50')
        ]);
        if (!docsRes.ok || !qualityRes.ok || !routingRes.ok) {
          throw new Error('Failed to fetch Data Lab telemetry');
        }
        const [docsJson, qualityJson, routingJson] = await Promise.all([
          readJson(docsRes),
          readJson(qualityRes),
          readJson(routingRes)
        ]);
        if (!isMounted) return;
        setPolicyDocs(Array.isArray(docsJson?.data?.docs) ? docsJson.data.docs : []);
        setQualitySummary(qualityJson?.data ?? null);
        setRoutingInsights(Array.isArray(routingJson?.data) ? routingJson.data : []);
      } catch (error) {
        if (!isMounted) return;
        setDataLabError(error instanceof Error ? error.message : 'Unable to load Data Lab telemetry');
      } finally {
        if (isMounted) setIsLoadingDataLab(false);
      }
    };
    void loadDataLab();
    return () => {
  ...
  return (
    <div className="page-shell max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {dataLabError && (
        <div className="rounded-sm border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-[11px] text-amber-300 uppercase tracking-wide">
          {dataLabError}
        </div>
      )}
      ...
      {activeTab === 'IMPORT' && (
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <div className="card-elevated p-6">
              <h3 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-4">Policy Doc Vault</h3>
              <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                {isLoadingDataLab && policyDocs.length === 0 && (
                  <p className="text-[10px] text-on-surface-variant">Loading policy documents...</p>
                )}
                {!isLoadingDataLab && policyDocs.length === 0 && (
                  <p className="text-[10px] text-on-surface-variant">No policy documents discovered in configured vault path.</p>
                )}
                {policyDocs.slice(0, 8).map((doc) => (
                  <div key={doc.id} className="rounded-sm border border-outline-variant/10 bg-surface-container p-3">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-[11px] font-bold text-on-surface truncate">{doc.fileName}</p>
                      <span className="text-[9px] uppercase tracking-widest text-primary">{doc.policyType}</span>
                    </div>
                    <p className="text-[10px] text-on-surface-variant mt-1">{doc.sizeLabel} • {new Date(doc.modifiedAt).toLocaleDateString()}</p>
                    <p className="text-[10px] text-on-surface-variant mt-1 truncate">{doc.coverageHints.join(', ')}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-6">
              <div className="flex items-center gap-2 text-primary mb-3">
                <Play size={16} />
                <h3 className="text-xs font-bold uppercase tracking-widest">Quality Snapshot</h3>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <MetricTile label="Docs Processed" value={qualitySummary ? `${qualitySummary.processedDocs}/${qualitySummary.totalDocs}` : '--'} />
                <MetricTile label="Completion" value={qualitySummary ? `${qualitySummary.completionRate}%` : '--'} />
                <MetricTile label="Verified" value={qualitySummary ? `${qualitySummary.verifiedDocs}` : '--'} />
                <MetricTile label="Pending Cases" value={qualitySummary ? `${qualitySummary.pendingCases}` : '--'} />
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">
                ...
                <th className="px-6 py-4">Selected Workflow</th>
                <th className="px-6 py-4">Routing Source</th>
                <th className="px-6 py-4">Confidence</th>
                <th className="px-6 py-4">Top Reason</th>
                <th className="px-6 py-4">Timestamp</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/10">
              {routingInsights.map((item) => (
                <tr key={item.runId} className="hover:bg-surface-container-high/35 transition-colors">
                  <td className="px-6 py-4">
                    <p className="text-[11px] font-mono text-primary">{item.caseId}</p>
                    <p className="text-[10px] text-on-surface-variant">{item.caseType ?? 'unknown'}</p>
                  </td>
                  <td className="px-6 py-4 text-[11px] font-bold text-on-surface">{item.selectedWorkflowName ?? 'Unresolved'}</td>
                  <td className="px-6 py-4 text-[11px] text-on-surface-variant uppercase">{item.source ?? 'n/a'}</td>
                  <td className="px-6 py-4 text-[11px] text-on-surface">{item.confidence ? `${Math.round(item.confidence * 100)}%` : '--'}</td>
                  <td className="px-6 py-4 text-[11px] text-on-surface-variant max-w-xs truncate">{item.reasons?.[0] ?? 'No reason recorded'}</td>
                  <td className="px-6 py-4 text-[11px] text-on-surface-variant font-mono">{new Date(item.at).toLocaleString()}</td>
                </tr>
              ))}
              {!isLoadingDataLab && routingInsights.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-[11px] text-on-surface-variant">
                    No workflow routing events yet. Send an inbox or portal request to generate trace rows.
                  </td>
                </tr>
              )}
            </tbody>
...
const MetricTile = ({ label, value }: { label: string; value: string }) => (
  <div className="rounded-sm border border-primary/20 bg-surface-container px-3 py-2">
